@extends('layouts.template')
	...
	@section('title')
		{{trans('copies.breadcrumbs.editar_destino')}}
	@endsection
	...
	@section('hist_navegacion')
		@component('components.historial_navegacion')    
			@slot('breadcrumbs', [
				...
				"active" 				=> trans('copies.breadcrumbs.editar_destino')
			])
		@endcomponent
	@endsection
		...
		<div class="col-lg-12">
			<div class="ibox float-e-margins">
				<div class="ibox-title border-bottom">
					<h5>
						{{ trans('copies.gestion_destinos.editar_destino') }}
						<small></small>
					</h5>
				</div>
				<div class="ibox-content">
					<form id="formCrearDestino" action="{{ route('destinos/guardar') }}" method="post" class="form-horizontal" enctype="multipart/form-data">
						...
						<input name="codigo" type="hidden" value="{{ $destino->codigo}}" />
						<!-- ubicación -->							
						...
						<div class="form-group">
							...
							<input type="text" class="form-control" name="descripcion_destino" id="descripcion_destino" value="{{$destino->descripcion}}" required>
							</div>
						</div>
						<!-- direccion -->
						<div class="form-group">
							...
							<div class="col-sm-6 col-md-9">									
								 <textarea class="form-control" name="direccion_destino" id="direccion_destino" rows="3" maxlength=50 required>{{$destino->direccion}}</textarea>
							</div>
						</div>
						<!-- kilometros -->						
						<div class="form-group">
						{{ Form::label('cant_km_recorrer', trans('copies.gestion_destinos.cant_km'), ['class' => 'col-sm-2 control-label']) }}
							...
							{{ Form::number('cant_km_recorrer', $destino->km_recorrer, ['class' => 'form-control required', 'placeholder' => '', 'maxlength' => '6', 'required'=>'required' ]) }}
							</div>
						</div>	
						<!-- dias -->						
						<div class="form-group">
						{{ Form::label('cant_dias_traslado', trans('copies.gestion_destinos.cant_dias'), ['class' => 'col-sm-2 control-label']) }}
							<div class="col-sm-3 col-md-3 col-lg-3">
							{{ Form::number('cant_dias_traslado',  $destino->cant_dias_traslado, ['class' => 'form-control required', 'placeholder' => '', 'maxlength' => '6', 'required'=>'required'  ]) }}
							</div>
						</div>	
						<!-- fotos -->						
						<div class="form-group">						 		
						...
						<div class="form-group text-right">
							<div class="col-sm-6 col-md-11">
								...
								<button type="reset" class="btn btn-secondary">
									{{ trans('copies.generales.boton_cancelar') }}
								</button>
							</div>
						</div>
					</form>
...
@section('codigo_scripts') 
	<script> 
			/*var pais = "{{$destino->get_cod_pais}}";
			var estado = "{{$destino->get_cod_estado}}";
			var ciudad = "{{$destino->cod_ciudad}}";*/
			var pais = 1;
			var estado = 1;
			var ciudad = 1;
			var ruta_sitio = "{{asset('/')}}";
			var ruta_adjuntos = "{{Config::get('constants.RUTA_FOTOS_DESTINO')}}";
			var array_rutas_adjuntos = new Array();
	</script>
	
	@foreach ($destino->getFotosDestino as $fotos)
		<script>
			array_rutas_adjuntos.push(ruta_sitio + ruta_adjuntos + "<?php echo $fotos->archivo; ?>");
		</script>	
	@endforeach 	

	<script src="{{ asset('/js/destinos/editar_destino.js')}}"></script>
@endsection
